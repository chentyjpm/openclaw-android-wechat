...
import com.ws.kimi_server.core.CoreForegroundService
import com.ws.kimi_server.core.ServiceStateStore
import com.ws.kimi_server.util.isAccessibilityEnabled
import com.ws.kimi_server.util.openAccessibilitySettings
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        openUsageAccess.setOnClickListener {
            com.ws.kimi_server.util.openUsageAccessSettings(this)
        }
        ...
        registerReceiver(accConnectedReceiver, IntentFilter(com.ws.kimi_server.acc.MyAccessibilityService.ACTION_CONNECTED))
        registerReceiver(accDisconnectedReceiver, IntentFilter(com.ws.kimi_server.acc.MyAccessibilityService.ACTION_DISCONNECTED))

        // Initialize debug switches from config and persist changes
        val cfg0 = com.ws.kimi_server.link.LinkConfigStore.load(this)
        ...
        debugSwitch.setOnCheckedChangeListener { _, isChecked ->
            val cur = com.ws.kimi_server.link.LinkConfigStore.load(this)
            com.ws.kimi_server.link.LinkConfigStore.save(this, cur.copy(debugEvents = isChecked))
        }
        debugXmlSwitch.setOnCheckedChangeListener { _, isChecked ->
            val cur = com.ws.kimi_server.link.LinkConfigStore.load(this)
            com.ws.kimi_server.link.LinkConfigStore.save(this, cur.copy(debugXml = isChecked))
        }
    }
    ...
    override fun onResume() {
        ...
        updateRecentPkg()
        updateServiceStateUi(ServiceStateStore.isRunning(this))
        val cfg = com.ws.kimi_server.link.LinkConfigStore.load(this)
    ...
    private fun updateRecentPkg() {
        val has = com.ws.kimi_server.util.isUsageAccessGranted(this)
        if (!has) {
            recentPkgText.text = "未授权"
            return
        }
        val pkg = com.ws.kimi_server.util.getLatestForegroundPackage(this, 60_000) ?: "未知"

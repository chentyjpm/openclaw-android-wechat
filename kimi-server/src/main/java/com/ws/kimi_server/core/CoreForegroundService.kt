...
import com.ws.kimi_server.link.http.ClientAck
import com.ws.kimi_server.link.http.ClientEnvelope
import com.ws.kimi_server.link.http.NavigateCommand
import com.ws.kimi_server.link.http.RestLink
import com.ws.kimi_server.link.http.SendTextCommand
import com.ws.kimi_server.link.http.ServerTask
import com.ws.kimi_server.link.LinkConfigStore
import com.ws.kimi_server.exec.TaskExecutor
import com.ws.kimi_server.ui.MainActivity
import com.ws.kimi_server.util.Logger
import com.ws.kimi_server.R
...
class CoreForegroundService : Service() {
    ...
    override fun onCreate() {
        ...
        com.ws.kimi_server.exec.TaskBridge.sendEnvelope = { envelope ->
            link.sendEnvelope(envelope)
        }
        executor = TaskExecutor()
        // Receive snapshots from AccessibilityService
        val filter = IntentFilter().apply { addAction(com.ws.kimi_server.acc.MyAccessibilityService.ACTION_SNAPSHOT) }
        snapshotReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                if (intent?.action == com.ws.kimi_server.acc.MyAccessibilityService.ACTION_SNAPSHOT) {
                    ...
                    val pkg = intent.getStringExtra(com.ws.kimi_server.acc.MyAccessibilityService.EXTRA_PKG) ?: ""
                    val cls = intent.getStringExtra(com.ws.kimi_server.acc.MyAccessibilityService.EXTRA_CLS) ?: ""
                    val text = intent.getStringExtra(com.ws.kimi_server.acc.MyAccessibilityService.EXTRA_TEXT) ?: ""
                    ...
                    val json = org.json.JSONObject().apply {
                        ...
                        val wx = intent.getStringExtra(com.ws.kimi_server.acc.MyAccessibilityService.EXTRA_WECHAT_JSON)
                        if (!wx.isNullOrBlank()) {
                    ...
                    val state = intent.getStringExtra(com.ws.kimi_server.acc.MyAccessibilityService.EXTRA_STATE_JSON)
    ...
    private fun handleNavigateCommand(nav: NavigateCommand) {
        ...
        val agent = com.ws.kimi_server.exec.AccessibilityAgent()
        val preferRecents = when {
            nav.preferRecents -> true
            pkg == com.ws.kimi_server.apps.wechat.WeChatSpec.PKG && nav.argsJson.isBlank() -> true
            else -> false
        }
        var ok = false
        if (preferRecents) {
    ...
    private fun handleSendTextCommand(cmd: SendTextCommand) {
        ...
        when (mode) {
            ...
            "latest_image" -> {
                val ensured = if (targetTitle.isNullOrBlank()) true else com.ws.kimi_server.apps.wechat.WeChatAgent.ensureChat(targetTitle)
                sendCommandAck(requestId, true, null, "QUEUED")
                val ok = if (ensured) {
                    com.ws.kimi_server.apps.wechat.WeChatAgent.sendLatestImage { stage ->
                        if (stage != com.ws.kimi_server.apps.wechat.WeChatAgent.ImageSendStage.SENT) {
                ...
                val (ok, error) = if (text.isNullOrBlank()) {
                    ...
                    if (!ensured) {
                        ...
                        val sendOk = com.ws.kimi_server.apps.wechat.WeChatAgent.sendText(text)
    ...
    private fun handleImageCommand(
        ...
        if (!targetTitle.isNullOrBlank()) {
            val ensureOk = com.ws.kimi_server.apps.wechat.WeChatAgent.ensureChat(targetTitle)
            if (!ensureOk) {
                ...
                if (bytes == null) {
                    ...
                    if (uri == null) {
                        ...
                        if (!persisted) {
                            ...
                            ok = com.ws.kimi_server.apps.wechat.WeChatAgent.sendLatestImage { stage ->
                                if (stage != com.ws.kimi_server.apps.wechat.WeChatAgent.ImageSendStage.SENT) {
                                    sendCommandAck(requestId, true, null, mapImageStage(stage))
                                }
                            }
    ...
    private fun mapImageStage(stage: com.ws.kimi_server.apps.wechat.WeChatAgent.ImageSendStage): String {
        return when (stage) {
            com.ws.kimi_server.apps.wechat.WeChatAgent.ImageSendStage.ATTACH_PANEL -> "ATTACH_PANEL"
            com.ws.kimi_server.apps.wechat.WeChatAgent.ImageSendStage.ALBUM_OPEN -> "ALBUM_OPEN"
            com.ws.kimi_server.apps.wechat.WeChatAgent.ImageSendStage.SELECTED -> "SELECTED"
            com.ws.kimi_server.apps.wechat.WeChatAgent.ImageSendStage.SENT -> "SENT"
        }
    }
    ...
    override fun onDestroy() {
        super.onDestroy()
        link.close()
        com.ws.kimi_server.exec.TaskBridge.sendEnvelope = null
    ...
    companion object {
        const val ACTION_LINK_STATE = "com.ws.kimi_server.LINK_STATE"
        const val EXTRA_STATE = "state"
        const val ACTION_LINK_ERROR = "com.ws.kimi_server.LINK_ERROR"
        const val EXTRA_ERROR = "error"
        const val ACTION_QUERY_STATE = "com.ws.kimi_server.QUERY_STATE"
